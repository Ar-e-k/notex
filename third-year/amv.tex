...
\begin{section}{Partial Differential Equations}
  ...
  \begin{subsection}{Clasiffication}

    Most generral second order PDEs in n independent variables
    \(x_{1}, \dots , x_{n}\) take the form \[\sum^{n}_{i, j = 0} A_{ij}
      \partial_{i} \partial_{j} u + \sum^{n}_{i = 1} B_{i} \partial_{i} u +Cu +
      D = 0,\] where \(A_{ij}, B_{i}, C, D\) are functions depending on
    \(x_{i}s\) as well as i and j. For any \(i, j\), \(A_{ij} = A_{ji}\),
    meaning A is a symmetric \(n \times n\) matrix.
